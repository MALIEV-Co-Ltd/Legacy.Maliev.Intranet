...
internal static class PurchaseOrdersEndpointMapper
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        using (response)
        {
            ...
            {
                return Results.Ok(new PurchaseOrderListPage([], emptyPageIndex, 0, 0, false, emptyPageIndex > 1));
            }
            ...
            try
            {
                var page = await response.Content.ReadFromJsonAsync<PurchaseOrderListPage>(cancellationToken);
                var invalid = page?.Items is null ||
                    page.PageIndex < 1 ||
                    page.TotalPages < 0 ||
                    page.TotalRecords < 0 ||
                    page.Items.Any(order => order.Id < 1 || order.EmployeeId is < 1);
